...
export class BurnGraphServiceBase {
  ...
  constructor(rootInput: string, options: BurnGraphServiceOptions = {}) {
    ...
    this.lifecycle = new RunLifecycle({
      database: this.database,
      runRow: (runId) => this.runRow(runId),
      descendantRunRows: (runId) => this.descendantRunRows(runId),
      appendEvent: (entry) => this.appendEvent(entry),
      getEvent: (sequence) => this.getEvent(sequence),
      getSnapshot: (reference, eventLimit) => this.getSnapshot(reference, eventLimit),
      bumpRun: (runId, at) => this.bumpRun(runId, at),
      settleAncestors: (runId, at) => this.settleAncestors(runId, at),
      driveStaticSubgraphs: (runId, at, changes, runtimeChanges) =>
        this.driveStaticSubgraphs(runId, at, changes, runtimeChanges),
      executeLifecycleMutation: (kind, reference, idempotencyKey, mutate) =>
        this.executeLifecycleMutation(kind, reference, idempotencyKey, mutate),
    });
