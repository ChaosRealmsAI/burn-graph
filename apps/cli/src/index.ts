...
// The version is read from the root manifest rather than restated here, so the
// installed CLI cannot claim a version the package does not have.
...
const program = new Command("burn-graph")
  // No process.cwd() default: an explicit default here would be echoed back as an
  // absolute private path in help and error output.
...
program
  ...
  .action((projectPath?: string) => {
    ...
    // "." rather than the resolved root: the caller already knows where they
    // are, and the absolute path is a private local path (P010).
...
function recoveryActions(error: BurnGraphError): readonly NextAction[] {
  // A malformed document is the failure a source-blind author hits first, so the
  // recovery is the complete schema and a known-good example rather than a
  // pointer at documentation that is not in the installed package.
  if (
